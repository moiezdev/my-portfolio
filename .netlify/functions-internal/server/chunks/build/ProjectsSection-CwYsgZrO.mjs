import { a as __nuxt_component_0$1, _ as __nuxt_component_0$2 } from './server.mjs';
import { defineComponent, reactive, mergeProps, withCtx, createTextVNode, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrRenderAttr, ssrInterpolate } from 'vue/server-renderer';

const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "ProjectsSection",
  __ssrInlineRender: true,
  props: ["type"],
  setup(__props) {
    const projects = reactive([
      {
        image: "scmborba.png",
        description: "",
        name: "Scmborba",
        link: "https://www.scmborba.pt/",
        tags: ["Laravel + Vue.js", "Keyframe Animations"]
      },
      {
        image: "aatourism.png",
        description: "",
        name: "AATourism",
        link: "https://www.aatourism.ca/",
        tags: ["Nuxt2", "Keyframe Animations", "Api Integrations"]
      },
      {
        image: "ain_saiss.png",
        description: "",
        name: "Ain Saiss",
        link: "https://ain-saiss.ma/",
        tags: ["Frontend", "Javascript Solutions"]
      },
      {
        image: "city_arrivals.png",
        description: "",
        name: "City Arrivals",
        link: "https://cityarrivals.ca/",
        tags: ["UX Development", "Javascript Solutions", "Api Integration"]
      }
    ]);
    return (_ctx, _push, _parent, _attrs) => {
      const _component_SectionTitle = __nuxt_component_0$1;
      const _component_NuxtLink = __nuxt_component_0$2;
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: [__props.type == "page" ? "min-h-screen" : "", "py-[40px]"]
      }, _attrs))}>`);
      if (__props.type !== "page") {
        _push(ssrRenderComponent(_component_SectionTitle, {
          title: "Featured Projects",
          subtitle: "Take a look at some of my recent work and creative solutions"
        }, null, _parent));
      } else {
        _push(ssrRenderComponent(_component_SectionTitle, {
          title: "My Projects",
          subtitle: "Take a look at some of my recent work and solutions"
        }, null, _parent));
      }
      _push(`<div class="grid grid-cols-3 dark:bg-dark-5 bg-darkMuted py-[50px] px-50 gap-4"><!--[-->`);
      ssrRenderList(projects, (project) => {
        _push(`<div class="dark:bg-darkBackground bg-darkButton col-span-1 rounded-3xl cursor-pointer overflow-hidden relative hover:z-10 hover:scale-105"><div class="relative"><a class=""${ssrRenderAttr("href", project.link)} target="_blank"><img${ssrRenderAttr("src", `/${project.image}`)} alt=""><svg class="absolute bottom-[10px] left-[10px]" width="27" height="27" viewBox="0 0 27 27" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><rect width="27" height="27" fill="url(#pattern0_2112_70)"></rect><defs><pattern id="pattern0_2112_70" patternContentUnits="objectBoundingBox" width="1" height="1"><use xlink:href="#image0_2112_70" transform="scale(0.00390625)"></use></pattern><image id="image0_2112_70" width="256" height="256" preserveAspectRatio="none" xlink:href="data:image/png;base64,"></image></defs></svg></a></div><div class="px-[25px] py-[20px]"><h1 class="text-[26px] mb-[15px] text-white">${ssrInterpolate(project.name)}</h1><div class="flex justify-baseline gap-2"><!--[-->`);
        ssrRenderList(project.tags, (tag) => {
          _push(`<div class="py-[3px] px-[5px] dark:bg-[#374151] not-dark:border-white border-transparent border-1 not-dark:text-white dark:text-darkCyan bg-darkButton rounded-full"><p class="text-[12px] font-medium">${ssrInterpolate(tag)}</p></div>`);
        });
        _push(`<!--]--></div></div></div>`);
      });
      _push(`<!--]--></div>`);
      if (__props.type !== "page") {
        _push(`<div class="flex justify-center">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/projects",
          class: "px-[25px] py-[15px] mt-[30px] rounded-full bg-darkCyan text-white font-display"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(` View All Projects `);
            } else {
              return [
                createTextVNode(" View All Projects ")
              ];
            }
          }),
          _: 1
        }, _parent));
        _push(`</div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
    };
  }
});
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/ProjectsSection.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const __nuxt_component_0 = Object.assign(_sfc_main, { __name: "ProjectsSection" });

export { __nuxt_component_0 as _ };
//# sourceMappingURL=ProjectsSection-CwYsgZrO.mjs.map
