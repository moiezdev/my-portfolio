<template>
  <div :class="type == 'page' ? 'min-h-screen' : ''" class="py-[40px]">
    <SectionTitle v-if="type !== 'page'" title="Featured Projects"
    ...
    <div class="grid grid-cols-3 @max-md:grid-cols-1 dark:bg-dark-5 bg-darkMuted py-[50px] px-50 @max-md:px-5 gap-4">
      <!--      projects card-->
      <motion.div v-for="project, index in projects" :inView="{ scale: [1.05, 1], opacity: [0.5, 1.0] }"
        class="dark:bg-darkBackground bg-darkButton col-span-1 rounded-3xl cursor-pointer overflow-hidden relative hover:z-10 hover:scale-105 transition-transform ease-in-out">
        <div class="relative">
        ...
        <div class="px-[25px] py-[20px]">
          ...
          <div class="flex flex-wrap justify-baseline gap-2">
            <div v-for="tag in project.tags"
              class="py-[3px] px-[5px] dark:bg-[#374151] not-dark:border-white border-transparent border-1 not-dark:text-white dark:text-darkCyan bg-darkButton rounded-full">
...
import { delay, Motion, motion } from "motion-v";
